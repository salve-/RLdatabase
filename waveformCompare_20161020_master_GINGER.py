...
surface waves) using broadband data from GIGS (GINGER) station for the
acceleration and from GINGERino ringlaser for the vertical
rotation rate.
...
def download_data(origin_time, net, sta, loc, chan, source):
    ...
    :param net: Network code, e.g. ``'BW'``.
    :type sta: str
    :param sta: Station code, e.g. ``'GIGS'``.
    :type loc: str
    :param loc: Location code, e.g. ``'01'``. Location code may
        contain wild cards.
    :type chan: str
    ...
    try:
        ...
        st = c.get_waveforms(network=net, station=sta, location='', channel=chan,
                             ...
                             endtime=origin_time+3*3600+10, attach_response=True)

    except:
        print "trying to fetch data from file..."        
        dataDir_get = '/import/two-data/simonelli/'
        fileName = ".".join((net, sta, "." + chan,
                             origin_time.strftime("%Y.%j")))
        filePath = os.path.join(dataDir_get, fileName)
        o_time2 = origin_time + 86400
        fileName2 = ".".join((net, sta, "." + chan,
                             o_time2.strftime("%Y.%j")))
        filePath2 = os.path.join(dataDir_get, fileName2)
        print(o_time2)
    ...
    ## sometimes hardware errors occur, so there are gaps in the streams, splitting them into smaller traces
    ## here we merge the traces to one large trace and fill the gaps with zeros! 
...
def event_info_data(event, station, mode):
    ...
    Ringlaser (GINGERino) and broadband signals (GIGS) are received from the
    download_data function.
    The great circle distance (in m and °) between event location and station
    in Gran Sasso, as well as the theoretical backazimuth are computed.

    :type event: :class: `~obspy.core.event.Event`
    :param event: Contains the event information.
    :type station: str
    :param station: Station from which data are fetched ('GIGS').
    :type mode: str
    :param mode: Defines if GIGS data are fetched from Neries ('neries')
    ...
    if station == 'GIGS' and mode == 'link':
        depth = origin.depth * 0.001  # Depth in km
    else:
        depth = origin.depth * 0.001  # Depth in km
    if station == 'GIGS':
        source = 'INGV' # if erde doesn't work, try 'BGR'
        net_r = 'IV'
        net_s = 'IV' #BW'
        sta_r = 'GNG'
        sta_s = 'GIGS'#'WETR'
        ...
        for acx in [acE,acN,acZ]:
            acx.remove_response(output='ACC', pre_filt=None, plot=False)
            acx.decimate(factor=5)
            if len(acx)>1:
                acx.merge(fill_value=0)
        ac = Stream(traces=[acE[0], acN[0], acZ[0]])
        for ca in [ac[0], ac[1], ac[2], rt[0]]:
            ca.stats.coordinates = AttribDict()
            ca.stats.coordinates['longitude'] = 13.572833
            ca.stats.coordinates['latitude'] = 42.453167
...
def station_components(station):
    ...
    """
    The East and North components have different labels in the GIGS and PFO
    data. They are standardized by this function.

    :type station: str
    :param station: Station from which data are fetched ('GIGS' or 'PFO').
    :rtype compE: str
...
def remove_instr_resp(rt, ac, rt_pcoda, ac_pcoda, station, startev):
    ...
    if station == 'GIGS':
        rt[0].data = rt[0].data * 1e9  # Rotation rate in nrad/s
        ac[0].data = ac[0].data * 1e9
        ac[1].data = ac[1].data * 1e9
        ac[2].data = ac[2].data * 1e9
...
def plotWaveformComp(event, station, link, mode, filter_e, event_source):
    ...
    tag_name = os.path.join('database/static/OUTPUT', catalog + '_' + str(event.origins[0]['time'].date) +
     'T' + str(event.origins[0]['time'].hour) + ':' + str(event.origins[0]['time'].minute) + '_' +
      str(event.magnitudes[0]['mag']) + '_' + 
     str(event.event_descriptions[0]['text'].splitlines()[0].replace(' ', '_').replace(',', '_')) + '___GINGER')
    tag_name2 = os.path.join('database/static/OUTPUT/Phase_velocities_GINGER_', str(event)
    ...
    if station == 'GIGS':
        ...
        if is_local(baz) == 'non-local':
            ...
            plt.text(statlon + 200000, statlat, 'GIGS', va="top",
            ...
            map.scatter(statlon, statlat, 200, color="b", marker="v",
                        edgecolor="k", zorder=100)
            plt.text(statlon + 35000, statlat, 'GIGS', fontsize=12, va="top",
                     family="monospace", weight="bold", zorder=101,
                     color='k', backgroundcolor='white')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--station', help='Choice of station: GIGS\
        (default is GIGS)', type=str, default='GIGS')
    parser.add_argument('--mode', help='Choice of executive mode for GIGS: \
    ...
    if station == 'GIGS' and mode == 'qmlfile':
        ...
        cat = c.get_events(minmagnitude=args.min_magnitude,
                           ...
                           catalog=catalog)
        cat2 = c2.get_events(starttime=args.min_datetime,
                           endtime=args.max_datetime,
                           latitude=42.453167,
                           longitude=13.572,
                           maxradiuskm=100,
    ...
    for event in cat:
        ...
        try:
            tag_name = os.path.join('database/static/OUTPUT', catalog + '_' + str(event.origins[0]['time'].date) + 
                    ...
                    '_' + str(event.event_descriptions[0]['text'].splitlines()[0].replace(' ', '_').replace(',', '_')) + '___GINGER')

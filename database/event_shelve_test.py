...
class EventShelve(object):
    def __init__(self, shelve_path, root_folder, quakeml_glob_expr,
        ...
        for _i, filename in enumerate(to_be_added):
            ...
            try:
                ...
                with open( filename[:-4] + "___GINGER." +'json') as data_file:
                    data = json.load(data_file)
    ...
    def query(self, station= None, starttime=None, endtime=None, minlatitude=None,
        ...
        for filename, event in self.events.iteritems():
            ...
            if station == "GINGER":
                if (event_id is None or event["event_id"] == event_id) and \
                        (station is None or station in filename) and \
                ...
                if (event_id is None or event["event_id"] == event_id) and \
                        (station is None or ("GINGER" not in filename)) and \
                        (starttime is None or event["time"] >= starttime) and \
                        (endtime is None or event["time"] <= endtime) and \
                        (minlatitude is None or
                         event["latitude"] >= float(minlatitude)) and \
        ...
        print "Found events:", len(found_events), station
            ...
            if len(found_events) == 0:
                ...
                if len(found_events.keys()) == 0:
                    ...
                    for ev in range(0,len(found_events.keys())):
                        if "GINGER" in found_events.keys()[ev]:
                            URL_P1= str(found_events.keys()[ev])[:-13] + '___GINGER_page_1.png'
                            URL_P2= str(found_events.keys()[ev])[:-13] + '___GINGER_page_2.png'
                            URL_P3= str(found_events.keys()[ev])[:-13] + '___GINGER_page_3.png'
                            URL_P4= str(found_events.keys()[ev])[:-13] + '___GINGER_page_4.png'
                        else:

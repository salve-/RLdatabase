...
including a distance-to-magnitude ratio.
Concerning dispersion curves there are different updated versions, that include
more frequency bands and improved filtering.
"""
...
def event_info_data(event, station, mode):
    ...
    baz = gps2DistAzimuth(latter, lonter, rt[0].stats.coordinates.latitude,
...
def remove_instr_resp(rt, ac, rt_pcoda, ac_pcoda, station, startev):
    ...
    if station == 'WET':
        ...
        taper1 = np.blackman(np.int(len(ac[1].data) * taper_percentage))
        taper1_left, taper1_right = np.array_split(taper1, 2)
        taper1 = np.concatenate([taper1_left,
                                np.ones(len(ac[1].data)-len(taper1)),
                                taper1_right])
        ac[1].data = ac[1].data * taper1

        taper2 = np.blackman(np.int(len(ac[2].data) * taper_percentage))
        taper2_left, taper2_right = np.array_split(taper2, 2)
        taper2 = np.concatenate([taper2_left,
                                np.ones(len(ac[2].data)-len(taper2)),
                                taper2_right])
        ac[2].data = ac[2].data * taper2
        ...
        print len(ac.data), len(ac_pcoda.data)
...
def ps_arrival_times(distance, depth, init_sec):
    ...
    traveltimes = getTravelTimes(
        delta=0.001 * distance / 111.11, depth=depth, model='ak135')
    tiemp = []
    tiems = []
    # from all possible P arrivals select the earliest one
    for i2 in xrange(0, len(traveltimes)):
        if traveltimes[i2]['phase_name'] == 'P' or traveltimes[i2][
                'phase_name'] == 'p' or traveltimes[i2]['phase_name'] ==\
                'Pdiff' or traveltimes[i2]['phase_name'] == 'PKiKP' or\
                traveltimes[i2]['phase_name'] == 'PKIKP' or traveltimes[i2][
                'phase_name'] == 'PP' or traveltimes[i2]['phase_name'] ==\
                'Pb' or traveltimes[i2]['phase_name'] == 'Pn' or\
                traveltimes[i2]['phase_name'] == 'Pg':
                    tiem_p = traveltimes[i2]['time']
                    tiemp.append(tiem_p)
    arriv_p = np.floor(init_sec + min(tiemp))

    # from all possible S arrivals select the earliest one
    for i3 in xrange(0, len(traveltimes)):
        if traveltimes[i3]['phase_name'] == 'S' or traveltimes[i3][
                'phase_name'] == 's' or traveltimes[i3]['phase_name'] ==\
                'Sdiff' or traveltimes[i3]['phase_name'] == 'SKiKS' or\
                traveltimes[i3]['phase_name'] == 'SKIKS' or traveltimes[i3][
                'phase_name'] == 'SS' or traveltimes[i3]['phase_name'] ==\
                'Sb' or traveltimes[i3]['phase_name'] == 'Sn' or\
                traveltimes[i3]['phase_name'] == 'Sg':
                    tiem_s = traveltimes[i3]['time']
                    tiems.append(tiem_s)
...
def store_info_json(rotate, ac, rt, corrcoefs, baz, arriv_p, EBA, tag_name,
                    ...
                    loc_r, loc_s, event_source, depth, magnitude, distance):
...
#    from IPython.core.debugger import Tracer; Tracer(colors="Linux")()
    ...
    outfile = open(tag_name + '/' + tag_name[11:] + '.json', 'wt')
...
def plotWaveformComp(event, station, link, mode, filter_e, event_source):
    ...
    tag_name = os.path.join('OUTPUT_new', catalog + '_' + str(event.origins[0]['time'].date) +
    ...
    tag_name2 = os.path.join('OUTPUT_new/Phase_velocities', str(event)
    ...
    if is_local(baz) == 'local':
        ...
        plt.title('Event: %s\n \n '
                  % (startev), fontsize=24, fontweight='bold')
        plt.subplot2grid((4, 9), (0, 4), colspan=5, rowspan=4)
        # conic map plot
        map = Basemap(projection='lcc', lat_0=(rt[0].stats.coordinates.
                      latitude + latter) / 2, lon_0=(rt[0].stats.coordinates.
    ...
    if station == 'WET':
        ...
        plt.subplot2grid((4, 9), (1, 0), colspan=2)
        plt.title(u'Event: %s \n' % (startev), fontsize=20, weight='bold')
        ax = plt.gca()
        ax.axis('equal')
        ax.axis('off')
    ...
    print 'Done.'
    plt.savefig(tag_name + '/' + tag_name[11:] + '_page_1.png')
    plt.close()
    ...
    rt, ac, rt_pcoda, ac_pcoda = remove_instr_resp(rt, ac, rt_pcoda, ac_pcoda,
                                                   station, startev)
    ...
    # TAPER
    taper_percentage = 0.05
    taper = np.blackman(np.int(len(rt[0].data) * taper_percentage))
    taper_left, taper_right = np.array_split(taper, 2)
    taper = np.concatenate([taper_left, np.ones(len(rt[0].data)-len(taper)),
                           taper_right])

    rt[0].data = rt[0].data * taper
    ...
    plt.annotate('4', xy=(min_lwf+xgap, box_yposition), fontsize=14,
                 fontweight='bold', bbox=bbox_props)
    plt.title(ur'Ring laser and broadband seismometer recordings. Event: %s'
              % (startev))
    plt.grid(True)
    ...
    plt.title(ur'4: Latter surface waves (Lowpass, cut-off: %s Hz)'
              % (cutoff))
    plt.grid(True)
    print 'Done.'
    plt.savefig(tag_name + '/' + tag_name[11:] + '_page_2.png')
    plt.close()
    ...
    plt.title(ur'Cross-correlation for $\dot\Omega_z$ and a$_T$ in %s seconds '
              'time windows (lowpass, cutoff: %s Hz). Event: %s'
              % (sec, cutoff, startev))
    plt.grid(True)
    plt.legend(shadow=True)
    ...
    cb1.set_label(ur'Max. x-corr. coeff.', fontweight='bold')

    plt.subplot2grid((4, 26), (2, 0), colspan=25)
    plt.plot(np.arange(0, sec * len(corrcoefs), sec), corrcoefs, '.k')
    plt.plot(np.arange(0, sec * len(corrcoefs) + 1, sec), thres, '--r', lw=2)
    plt.ylabel(ur'zero-lag x-corr. coeff.', fontsize=10, fontweight='bold')
    ...
    print 'Done.'
    plt.savefig(tag_name + '/' + tag_name[11:] + '_page_3.png')
    plt.close()
    ...
    # TAPER
    taper_percentage = 0.05
    taper2 = np.blackman(np.int(len(rt_pcoda[0].data) * taper_percentage))
    taper2_left, taper2_right = np.array_split(taper2, 2)
    taper2 = np.concatenate([taper2_left,
                             np.ones(len(rt_pcoda[0].data)-len(taper2)),
                             taper2_right])

    rt_pcoda[0].data = rt_pcoda[0].data * taper2
    ...
    plt.title(ur'$\dot{\mathbf{\Omega}}_\mathbf{z}$ and a$_\mathbf{T}$'
              'correlation in the P-coda in a 2 seconds time window'
              ' (highpass, cutoff: 1 Hz). Event: %s' % (startev))
    plt.axvline(x=min_pw, linewidth=1)
    plt.axvline(x=min_sw, linewidth=1)
    ...
    plt.ylabel(ur'Max. x-corr. coeff.', fontweight='bold')
    plt.xlim(0, (min_lwi + max_lwi) // 2)
    plt.ylim(0, 1)
    plt.grid(True)
    ...
    plt.savefig(tag_name + '/' + tag_name[11:] + '_page_4.png')
    plt.close()
    ...
    store_info_json(rotate, ac, rt, corrcoefs, baz, arriv_p, EBA, tag_name,
                    station, phasv_means, phasv_stds, startev, event, net_r,
                    net_s, chan1, chan2, chan3, chan4, sta_r, sta_s, source,
                    loc_r, loc_s, event_source, depth, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', help='Choice of executive mode for WET: \
        fetch event data from fdsnClient or get it from a link (for plotting\
    ...
    parser.add_argument('--min_magnitude', help='Minimum magnitude for \
        events (default is 3).', type=float or int, default=3.0)
    ...
    parser.add_argument('--min_datetime', help='Earliest date and time for \
        ...
        Example: 2010-02-27T05:00', type=str, default=str(datetime.datetime.now()-datetime.timedelta(hours=72)))
    ...
    if not os.path.exists('OUTPUT_new'):
        os.makedirs('OUTPUT_new')
    if not os.path.exists('OUTPUT_new/Phase_velocities'):
        os.makedirs('OUTPUT_new/Phase_velocities')
    ...
    for event in cat:
        ...
        if os.path.exists(str(tag_name)):
            ...
            event.write(tag_name + "/" + catalog + '_' + 
                str(event.origins[0]['time'].date) + 'T' + str(event.origins[0]['time'].hour) + ':' + str(event.origins[0]['time'].minute) +'_' + str(event.magnitudes[0]['mag']) + 
                '_' + str(event.event_descriptions[0]['text'].splitlines()[0].replace(' ', '_').replace(',', '_')) + 
                ".xml", format="QUAKEML")
            try:
